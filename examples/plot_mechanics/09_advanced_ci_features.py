...
results = fitter.ci_bounds(
    ci_levels=[68, 90, 95, 99],
    n_bootstrap=1000,  # Fast enough for high bootstrap counts!
    overall_ci=True,
    seed=42
)
...
results_extended = fitter.ci_bounds(
    ci_levels=[68, 95],
    n_bootstrap=1000,
    overall_ci=True,
    x_range=x_extended,
...
results_individual = fitter.ci_bounds(
    ci_levels=95,
    n_bootstrap=1000,
    overall_ci=False,
    individual_ci=True,
...
ax3.set_title(
    "Individual Component CIs (95%)\n"
    "Separate confidence bands for each model component",
    fontsize=12
)
ax3.set_xlabel("X")
ax3.set_ylabel("Y")
...
fig4.suptitle(
    "Both CI Types Computed in Single Call (overall_ci=True, individual_ci=True)",
    fontsize=13
)
plt.tight_layout()
