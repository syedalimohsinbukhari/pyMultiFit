...
y_g = multi_gaussian(x=x_g, params=params_g, noise_level=0.2)

params_m = [(-0.1, 3), (8, -15, 3), (6, 5, 2), (4, 20, 4)]
x_m = np.linspace(-35, 35, 2000)
y_m = multiple_models(x=x_m, params=params_m, model_list=[LINE, GAUSSIAN, LAPLACE, GAUSSIAN], noise_level=0.1)

# -- fit -----------------------------------------------------------------------
gf = GaussianFitter(x_values=x_g, y_values=y_g)
gf.fit([(10, -18, 1), (4, -5.5, 10), (5, -1, 0.5), (10, 3, 1), (4, 15, 3)])

mf = MixedDataFitter(x_values=x_m, y_values=y_m, model_list=[LINE, GAUSSIAN, LAPLACE, GAUSSIAN])
...
gf.plotter.plot_fit(
    ...
    x_label="X data",
    y_label="Amplitude",
    plot_title="5-component Gaussian fit | custom labels",
...
mf.plotter.plot_fit(
    show_individuals=True,
    x_label="X Data",
    plot_title="Mixed model fit  (Line + Gaussian + Laplace + Gaussian) | default + custom labels",
    axis=ax3,
)
plt.tight_layout()
