...
# Shared x domain so the location shift along the x-axis is accurately visualized
x_values = np.linspace(start=0, stop=8, num=500)

y_multifit = ChiSquareDistribution(degree_of_freedom=1, loc=0, normalize=True)
y_scipy = chi2

f, ax = plt.subplots(1, 2, figsize=(12, 5))

ax[0].plot(x_values, y_scipy.pdf(x=x_values, df=1, loc=0), label="Scipy Chi2")
ax[0].plot(x_values, y_multifit.pdf(x_values), "k:", label="pyMultiFit Chi2")
ax[0].set_ylabel("f(x)")

ax[1].plot(x_values, y_scipy.cdf(x=x_values, df=1, loc=0), label="Scipy Chi2")
ax[1].plot(x_values, y_multifit.cdf(x_values), "k:", label="pyMultiFit Chi2")
ax[1].set_ylabel("F(x)")

f.suptitle(r"$\chi^2$(1, loc=0)")
