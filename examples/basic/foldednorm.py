...
from src.pymultifit.distributions import FoldedNormalDistribution
...
y_multifit = FoldedNormalDistribution(sigma=3, normalize=True)

f, ax = plt.subplots(1, 2, figsize=(12, 5))

ax[0].plot(x_values, y_scipy.pdf(x=x_values, c=0, loc=3), label='Scipy translated Folded Normal')
ax[0].plot(x_values, y_multifit.pdf(x_values), 'k:', label='pyMultiFit translated Folded Normal')
ax[0].set_ylabel('f(x)')

ax[1].plot(x_values, y_scipy.cdf(x=x_values, c=0, loc=3), label='Scipy translated Folded Normal')
