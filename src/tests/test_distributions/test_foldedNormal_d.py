...
class TestFoldedNormalDistribution:
    ...
    def test_pdf_cdf():
        ...
        def _cdf_pdf_scipy(x_, loc, scale, what='cdf'):
            c = abs(loc) / scale
            return foldnorm.cdf(x_, c=c, loc=0, scale=scale) if what == 'cdf' else foldnorm.pdf(x_, c=c, loc=0, scale=scale)
        ...
        for i in ['pdf', 'cdf']:
            for _ in range(50):  # Run 50 random tests
                ...
                distribution = FoldedNormalDistribution(mean=loc_, sigma=scale_, normalize=True)
